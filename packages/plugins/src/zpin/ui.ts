...
function render() {
    const target = document.querySelector<HTMLDivElement>(".positionlist__list");
    if (!target) {
        return;
    }
...
function renderJob(root: HTMLElement) {
    ...
    const companyNameNode = root.querySelector<HTMLDivElement>(".companyinfo__name");
    const companyName = companyNameNode?.attributes.getNamedItem("title")?.value.trim() ?? "";
    const jobNameNode = root.querySelector<HTMLDivElement>(".jobinfo__name");
...
function enhanceJobAnalyzeView(root: HTMLElement, data: { [key: string]: string }) {
    ...
    const tagsContainter = root.querySelector<HTMLDivElement>(".jobinfo__other-info");
    if (!tagsContainter) {
        return;
    }
    ...
    promptTag.className = "jobinfo__other-info-item";
    ...
    tagsContainter.append(promptTag);
}
...
function enhanceJobView(root: HTMLElement, rule: ZeltoRule) {
    ...
    const chatJob = root.querySelector<HTMLDivElement>(".c-chat-job");
    if (chatJob) {
        chatJob.style.background = color;
    }
    const jobBtn = root.querySelector<HTMLDivElement>(".collect-and-apply__btn");
    if (jobBtn) {
        jobBtn.style.background = color;
    }
    ...
    keywordTag.className = "jobinfo__other-info-item";
    keywordTag.style.color = tagColor;
    keywordTag.innerText = (rule.keyword ?? "") + "-" + (rule.reason ?? "");
    tagsContainter.append(keywordTag);
}
